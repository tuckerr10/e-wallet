...
class _PinPageState extends State<PinPage> {
  ...
  void _onPinComplete(String pin) {
    ...
    if (kind == AppConstants.txnTopup) {
      ...
      // Direct payment using dummy OTP bypass
  ...
  Future<void> _handleBagStoreSync(int transactionId) async {
      ...
      try {
        ...
        if (tokenToUse != null) {
          ...
          if (ref.startsWith('single:')) {
            // Format: single:product_id:quantity
        ...
        debugPrint('[PinPage] Error sinkronisasi transaksi ke Bag Store: $e');
      }
    }
    ...
    if (mounted) {
      final result = (context.read<PaymentBloc>().state as PaymentTransferSuccess).result;
        ...
        BlocListener<PaymentBloc, PaymentState>(
          listener: (context, state) {
            if (state is PaymentTransferSuccess) {
              ...
              _handleBagStoreSync(result.transactionId);
